...
def test_main(mocker, given_args, config_values, dir_exists_values):
    m_ArgumentParser = mocker.patch("p4_templates.p4_template_tool.ArgumentParser", return_value=MockArgumentParser(given_args))
    m_os_path_exists = mocker.patch("p4_templates.p4_template_tool.os.path.exists", side_effect=dir_exists_values)
    m_os_path_abspath = mocker.patch("p4_templates.p4_template_tool.os.path.abspath", return_value='abs_path')
    m_load_server_config = mocker.patch("p4_templates.p4_template_tool.load_server_config", return_value=config_values)
    m_os_path_dirname = mocker.patch("p4_templates.p4_template_tool.os.path.dirname", return_value='a/dir/name/')
    m_os_chdir = mocker.patch("p4_templates.p4_template_tool.os.chdir")
    m_QApplication = mocker.patch("p4_templates.p4_template_tool.QApplication")
    m_P4TemplateLoaderDialog = mocker.patch("p4_templates.p4_template_tool.P4TemplateLoaderDialog")
